...
def add_rezervare(
    ...
    rezervare_data = RezervareCreate(id_client=id_client, data=data)
...
def update_rezervare(
    ...
    from datetime import datetime
    try:
        parsed_data = datetime.fromisoformat(data)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date format")

    rezervare_data = RezervareCreate(id_client=id_client, data=parsed_data)
