"""Rute CRUD pentru fragmentul LOCAL BUCURESTI (filtrat din vederi globale bdd_global)."""
...
def ref_servicii(db: Session = Depends(get_db)):
    return crud.get_servicii(db)


@router.get("/referinta/clienti")
def ref_clienti(db: Session = Depends(get_db)):
