...
router = APIRouter(prefix="/local/buc", tags=["Local – Bucuresti"])

_ORAS = "Bucuresti"

@router.get("/hoteluri")
def list_hoteluri(db: Session = Depends(get_db_buc)):
...
def remove_hotel(id_hotel: int, db: Session = Depends(get_db_buc)):
    crud.delete_hotel(db, id_hotel, _ORAS)
    return {"success": True}

@router.get("/angajati")
def list_angajati(db: Session = Depends(get_db_buc)):
...
def remove_angajat(id_angajat: int, db: Session = Depends(get_db_buc)):
    crud.delete_angajat(db, id_angajat, _ORAS)
    return {"success": True}

@router.get("/camere")
def list_camere(db: Session = Depends(get_db_buc)):
...
def remove_camera(id_camera: int, db: Session = Depends(get_db_buc)):
    crud.delete_camera(db, id_camera, _ORAS)
    return {"success": True}

@router.get("/referinta/tipuri_camera")
def ref_tip_camere(db: Session = Depends(get_db_buc)):
    return crud.get_tip_camere(db)

@router.get("/referinta/departamente")
def ref_departamente(db: Session = Depends(get_db_buc)):
    return crud.get_departamente(db)

@router.get("/referinta/servicii")
def ref_servicii(db: Session = Depends(get_db_buc)):
    return crud.get_servicii(db)

@router.get("/referinta/clienti")
def ref_clienti(db: Session = Depends(get_db_buc)):
    return crud.get_clienti(db)

@router.get("/clienti")
def list_clienti(db: Session = Depends(get_db_buc)):
    return crud.get_clienti(db)
...
def remove_client(id_client: int, db: Session = Depends(get_db_buc)):
    crud.delete_client_central(db, id_client)
    return {"success": True}

@router.get("/rezervari")
def list_rezervari(db: Session = Depends(get_db_buc)):
...
def remove_rezervare(id_rezervare: int, db: Session = Depends(get_db_buc)):
    crud.delete_rezervare_central(db, id_rezervare)
    return {"success": True}

@router.get("/plati")
def list_plati(db: Session = Depends(get_db_buc)):
...
def remove_plata(id_plata: int, db: Session = Depends(get_db_buc)):
    crud.delete_plata_central(db, id_plata)
    return {"success": True}

@router.get("/servicii")
def list_servicii(db: Session = Depends(get_db_buc)):
    return crud.get_servicii(db)
...
def remove_serviciu(id_serviciu: int, db: Session = Depends(get_db_buc)):
    crud.delete_serviciu_central(db, id_serviciu)
    return {"success": True}

@router.get("/departamente")
def list_departamente(db: Session = Depends(get_db_buc)):
    return crud.get_departamente(db)
...
def remove_departament(id_departament: int, db: Session = Depends(get_db_buc)):
    crud.delete_departament_central(db, id_departament)
    return {"success": True}

@router.get("/tipuri_camera")
def list_tipuri_camera(db: Session = Depends(get_db_buc)):
...
def remove_tip_camera(id_tip_camera: int, db: Session = Depends(get_db_buc)):
